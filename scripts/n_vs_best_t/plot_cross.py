...
def plot_heatmap(bt: pd.DataFrame, stratum: str, N: int, out_path: Path) -> None:
    sub = bt[(bt.stratum == stratum) & (bt.N == N)]
    if sub.empty:
        return
    pivot_t = sub.pivot(index="model", columns="dataset", values="t_star")
    pivot_acc = sub.pivot(index="model", columns="dataset", values="best_t_mean")
    ...
    ax.set_title(f"T*  at N={N}, stratum={stratum}")
...
def plot_grid_trajectories(bt: pd.DataFrame, stratum: str, out_path: Path) -> None:
    ...
    for i, model in enumerate(MODELS_ORDER):
        for j, dataset in enumerate(DATASETS):
            ...
            if s.empty:
                ...
                ax.set_xticks([])
                continue
            for col, color in [("best_t_mean", "C0"), ("acc_t1p0_mean", "C3"),
                               ("acc_t0p1_mean", "C2"), ("acc_random_t_mean", "C1"),
                               ("acc_equal_mix_mean", "C4")]:
...
def plot_win_margin_scatter(bt: pd.DataFrame, out_path: Path) -> None:
    """For each (model, dataset, stratum), x=gap_vs_equal_mix@N=16, y=@N=256."""
...
def main() -> None:
    ...
    for stratum in ["overall", "L5"]:
        for N in [64, 256]:
            plot_heatmap(bt, stratum, N, args.out_dir / f"D_heatmap_{stratum}_N{N}.png")
    plot_grid_trajectories(bt, "overall", args.out_dir / "E_grid_overall.png")
    plot_grid_trajectories(bt, "L5", args.out_dir / "E_grid_L5.png")
